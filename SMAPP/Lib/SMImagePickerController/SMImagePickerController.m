...
@property (nonatomic, weak) SMCameraViewController *camera;
...
- (void)viewDidLoad {
    ...
    if (self.sourceType == SMImagePickerControllerSourceTypeCamera) {
        
        self.statusBarHidden = YES;
        
        SMCameraViewController *camera = [[SMCameraViewController alloc] initWithPicker:self];
...
- (UINavigationController *)navigationController {
    if (!_navigationController) {
        _navigationController = [[UINavigationController alloc] initWithRootViewController:[[SMAlbumsViewController alloc] initWithPicker:self]];
...
- (void)selectAsset:(PHAsset *)asset {
    ...
    if (self.allowsMultipleSelection) {
        ...
        SMImageClipViewController *clip = [[SMImageClipViewController alloc] initWithImage:nil picker:self];
...
- (void)updateDoneButton {
    for (UIViewController *viewController in self.navigationController.viewControllers) {
        ...
        if (viewController.navigationItem.rightBarButtonItems.count > 1) {
            ...
            SMBadgeView *badgeView = badgeButtonItem.customView;
